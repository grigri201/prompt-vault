# github-auth-process - Task 3.1

Execute task 3.1 for the github-auth-process specification.

## Task Description
创建 GitHub 客户端接口 (需求: US3, D1)
...
```
Use the spec-task-executor agent to implement task 3.1: "创建 GitHub 客户端接口 (需求: US3, D1)" for the github-auth-process specification.
...
3. Implement ONLY task 3.1: "创建 GitHub 客户端接口 (需求: US3, D1)"
...
npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 3.1 --mode single
...
Task details:
- ID: 3.1
- Description: 创建 GitHub 客户端接口 (需求: US3, D1)
```

**Fallback Execution**: If the agent is not available, you can execute:
```
/spec-execute 3.1 github-auth-process
```
...
2. Execute task 3.1: "创建 GitHub 客户端接口 (需求: US3, D1)"
...
1. **Mark task complete**: Use the get-tasks script to mark completion:
   ```bash
   npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 3.1 --mode complete
   ```
2. **Confirm to user**: State clearly "Task 3.1 has been marked as complete"
...
```
Use the spec-task-implementation-reviewer agent to review the implementation of task 3.1 for the github-auth-process specification.
...
- Implementation changes for task 3.1
```
...
```
Use the spec-duplication-detector agent to analyze code duplication for task 3.1 of the github-auth-process specification.
...
```
Use the spec-integration-tester agent to test the implementation of task 3.1 for the github-auth-process specification.
