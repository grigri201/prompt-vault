# github-auth-process - Task 6.2

Execute task 6.2 for the github-auth-process specification.
...
```
Use the spec-task-executor agent to implement task 6.2: "更新根命令注册 (需求: TC3)" for the github-auth-process specification.
...
3. Implement ONLY task 6.2: "更新根命令注册 (需求: TC3)"
...
npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 6.2 --mode single
...
2. Execute task 6.2: "更新根命令注册 (需求: TC3)"
...
1. **Mark task complete**: Use the get-tasks script to mark completion:
   ```bash
   npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 6.2 --mode complete
   ```
2. **Confirm to user**: State clearly "Task 6.2 has been marked as complete"
...
```
Use the spec-task-implementation-reviewer agent to review the implementation of task 6.2 for the github-auth-process specification.
...
- Implementation changes for task 6.2
```
...
```
Use the spec-duplication-detector agent to analyze code duplication for task 6.2 of the github-auth-process specification.
...
```
Use the spec-integration-tester agent to test the implementation of task 6.2 for the github-auth-process specification.
