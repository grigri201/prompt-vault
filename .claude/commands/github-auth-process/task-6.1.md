# github-auth-process - Task 6.1

Execute task 6.1 for the github-auth-process specification.
...
```
Use the spec-task-executor agent to implement task 6.1: "更新 Wire 配置 (需求: TC3)" for the github-auth-process specification.
...
3. Implement ONLY task 6.1: "更新 Wire 配置 (需求: TC3)"
...
npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 6.1 --mode single
...
Task details:
- ID: 6.1
- Description: 更新 Wire 配置 (需求: TC3)
```

**Fallback Execution**: If the agent is not available, you can execute:
```
/spec-execute 6.1 github-auth-process
```
...
2. Execute task 6.1: "更新 Wire 配置 (需求: TC3)"
...
1. **Mark task complete**: Use the get-tasks script to mark completion:
   ```bash
   npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 6.1 --mode complete
   ```
2. **Confirm to user**: State clearly "Task 6.1 has been marked as complete"
...
```
Use the spec-task-implementation-reviewer agent to review the implementation of task 6.1 for the github-auth-process specification.
...
- Implementation changes for task 6.1
```
...
```
Use the spec-duplication-detector agent to analyze code duplication for task 6.1 of the github-auth-process specification.
...
```
Use the spec-integration-tester agent to test the implementation of task 6.1 for the github-auth-process specification.
