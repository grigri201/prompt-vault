# github-auth-process - Task 7.1

Execute task 7.1 for the github-auth-process specification.
...
```
Use the spec-task-executor agent to implement task 7.1: "创建配置存储单元测试 (需求: TC2)" for the github-auth-process specification.
...
3. Implement ONLY task 7.1: "创建配置存储单元测试 (需求: TC2)"
...
npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 7.1 --mode single
...
Task details:
- ID: 7.1
- Description: 创建配置存储单元测试 (需求: TC2)
```

**Fallback Execution**: If the agent is not available, you can execute:
```
/spec-execute 7.1 github-auth-process
```
...
2. Execute task 7.1: "创建配置存储单元测试 (需求: TC2)"
...
1. **Mark task complete**: Use the get-tasks script to mark completion:
   ```bash
   npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 7.1 --mode complete
   ```
2. **Confirm to user**: State clearly "Task 7.1 has been marked as complete"
...
```
Use the spec-task-implementation-reviewer agent to review the implementation of task 7.1 for the github-auth-process specification.
...
- Implementation changes for task 7.1
```
...
```
Use the spec-duplication-detector agent to analyze code duplication for task 7.1 of the github-auth-process specification.
...
```
Use the spec-integration-tester agent to test the implementation of task 7.1 for the github-auth-process specification.
