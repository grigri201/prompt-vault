# github-auth-process - Task 5.2

Execute task 5.2 for the github-auth-process specification.

## Task Description
实现 login 子命令 (需求: AC1, US1)
...
```
Use the spec-task-executor agent to implement task 5.2: "实现 login 子命令 (需求: AC1, US1)" for the github-auth-process specification.
...
3. Implement ONLY task 5.2: "实现 login 子命令 (需求: AC1, US1)"
...
npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 5.2 --mode single
...
Task details:
- ID: 5.2
- Description: 实现 login 子命令 (需求: AC1, US1)
```

**Fallback Execution**: If the agent is not available, you can execute:
```
/spec-execute 5.2 github-auth-process
```
...
2. Execute task 5.2: "实现 login 子命令 (需求: AC1, US1)"
...
1. **Mark task complete**: Use the get-tasks script to mark completion:
   ```bash
   npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 5.2 --mode complete
   ```
2. **Confirm to user**: State clearly "Task 5.2 has been marked as complete"
...
```
Use the spec-task-implementation-reviewer agent to review the implementation of task 5.2 for the github-auth-process specification.
...
- Implementation changes for task 5.2
```
...
```
Use the spec-duplication-detector agent to analyze code duplication for task 5.2 of the github-auth-process specification.
...
```
Use the spec-integration-tester agent to test the implementation of task 5.2 for the github-auth-process specification.
