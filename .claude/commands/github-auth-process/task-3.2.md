# github-auth-process - Task 3.2

Execute task 3.2 for the github-auth-process specification.

## Task Description
实现 GitHub API 调用 (需求: AC2, SC3, PR1)
...
```
Use the spec-task-executor agent to implement task 3.2: "实现 GitHub API 调用 (需求: AC2, SC3, PR1)" for the github-auth-process specification.
...
3. Implement ONLY task 3.2: "实现 GitHub API 调用 (需求: AC2, SC3, PR1)"
...
npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 3.2 --mode single
...
Task details:
- ID: 3.2
- Description: 实现 GitHub API 调用 (需求: AC2, SC3, PR1)
```

**Fallback Execution**: If the agent is not available, you can execute:
```
/spec-execute 3.2 github-auth-process
```
...
2. Execute task 3.2: "实现 GitHub API 调用 (需求: AC2, SC3, PR1)"
...
1. **Mark task complete**: Use the get-tasks script to mark completion:
   ```bash
   npx @pimzino/claude-code-spec-workflow@latest get-tasks github-auth-process 3.2 --mode complete
   ```
2. **Confirm to user**: State clearly "Task 3.2 has been marked as complete"
...
```
Use the spec-task-implementation-reviewer agent to review the implementation of task 3.2 for the github-auth-process specification.
...
- Implementation changes for task 3.2
```
...
```
Use the spec-duplication-detector agent to analyze code duplication for task 3.2 of the github-auth-process specification.
...
```
Use the spec-integration-tester agent to test the implementation of task 3.2 for the github-auth-process specification.
